...
# https://gist.github.com/sjlongland/3b89ce91d065851b292a#file-amqp-py-L301
...
class AMQPConnection(AMQPObject):
    ...
    def add_on_open_callback(self, callback, callback_args=None, callback_kwargs=None):
        self._on_open_cb.append(
                self._get_callback(callback, callback_args, callback_kwargs))

    def add_on_open_error_callback(self, callback, callback_args=None, callback_kwargs=None):
        self._on_open_error_cb.append(
                self._get_callback(callback, callback_args, callback_kwargs))

    def add_on_close_callback(self, callback, callback_args=None, callback_kwargs=None):
        self._on_close_cb.append(
                self._get_callback(callback, callback_args, callback_kwargs))

    def add_on_giveup_callback(self, callback, callback_args=None, callback_kwargs=None):
    ...
    def connect(self, timeout=None):
        ...
        return self._run_async(
            self._amqp_connection.connect, (), {}, timeout, None, None)
        ...
        try:
            ...
            for cb in self._on_close_cb:
                try:
                    ...
                    log.exception('Exception in callback %s(*%s, **%s)', cb, args, kwargs)
        except:
            log.exception('Exception in close callback')

        self._reconnect()

    def _handle_on_open_error_callback(self, connection, error, *args, **kwargs):
        """
    ...
    def _reconnect(self):
        ...
        try:
            ...
            if self._reconnect:
                ...
                log.error('Re-connecting in %s.', self._reconnect_delay)
            elif self._giveup:
                log.fatal('Giving up!')
                ...
                if cb_func is None:
                    ...
                    if return_result:
                        ...
                        try:
                            ...
                            log.exception('Exception running %s', async_func)
            ...
            try:
                ...
                if self._io_loop is callback_ioloop:
                    ...
                    if isinstance(cb_res, tornado.concurrent.Future):
                        # log.debug('Function is asynchronous, waiting for result')
                        @tornado.gen.coroutine
...
class AMQPChannel(AMQPObject):
    ...
    def add_callback(self, callback, replies, one_shot=True):
        return self._connection._run_async(
            ...
            args=(self._connection._get_callback(callback), replies, one_shot),
    ...
    def basic_cancel(self, callback=None, consumer_tag='', nowait=False):
        return self._connection._run_async(
            ...
            args=(self._connection._get_callback(callback), consumer_tag, nowait))
    ...
    def basic_get(self, callback, queue='', no_ack=False):
        return self._connection._run_async(
            ...
            args=(self._connection._get_callback(callback), queue, no_ack))

    def basic_nack(self, delivery_tag=None, multiple=False, requeue=True):
        return self._connection._run_async(
            self._channel.basic_nack, args=(delivery_tag, multiple, requeue))
    ...
    def basic_qos(self, callback=None, prefetch_size=0, prefetch_count=0, all_channels=False):
        return self._connection._run_async(
            ...
            args=(self._connection._get_callback(callback), prefetch_size, prefetch_count, all_channels))

    def basic_reject(self, delivery_tag=None, requeue=True):
        return self._connection._run_async(
            self._channel.basic_reject, args=(delivery_tag, requeue))

    def basic_recover(self, callback=None, prefetch_size=0, prefetch_count=0, all_channels=False):
        return self._connection._run_async(
            self._channel.basic_qos, args=(self._connection._get_callback(callback),
            prefetch_size, prefetch_count, all_channels))

    def close(self, *args, **kwargs):
        return self._connection._run_async(self._channel.close, args=args, kwargs=kwargs)
    ...
    def exchange_delete(self, exchange=None, if_unused=False, nowait=False, timeout=None):
    ...
    def flow(self, callback, active):
        return self._connection._run_async(
            self._channel.flow, args=(self._connection._get_callback(callback), active))
    ...
    def queue(self, queue=None, passive=False, durable=False,
        ...
        q = AMQPQueue(self, queue, passive, durable, exclusive, auto_delete, nowait, arguments)
        yield q.declare(timeout)
...
if __name__ == '__main__':
    ...
    def on_connect(*args, **kwargs):
        try:
            ...
            ex = yield rx_ch.exchange_declare(exchange='test', exchange_type='fanout')
            ...
            bind = yield rx_ch.queue_bind(queue=rx_q.method.queue, exchange='test', routing_key='#')
