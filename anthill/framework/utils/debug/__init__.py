...
class ExceptionReporter:
    ...
    def __init__(self, app=None, handler=None, exc_info=None, is_email=False):
        self.handler = handler
        self.app = app
        self.is_email = is_email

        if not any([app, handler]):
            raise ImproperlyConfigured('`handler` or `app` must have a value')

        if app is None and handler is not None:
            self.app = handler.application.app
